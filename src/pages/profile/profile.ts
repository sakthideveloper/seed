...
export class ProfilePage implements OnInit {
    ...
    onFilterOptions($event: any) {
        let $this = this;
        let course_id = $this.filterby;  // stores course_id
        if (course_id) {
            ...
            toast1.present();
            $this.attendance.get_user_course_attendance(course_id).then((res: any) => {
                console.log(res);
                if (res.message) {
                    let toast = this.toastCtrl.create({
                        message: res.message,
                ...
                if (status.authorized) {
                    ...
                    this.scanSub = this.qrScanner.scan()
                        .subscribe((textFound: string) => {
                            ...
                            let user_id = parseInt(textFound.match(/\d+/)[0]);
                            if (user_id) {
                                let toast1 = this.toastCtrl.create({
                                ...
                                toast1.present();
                                $this.attendance.marktodays_attendance(user_id).then((res: any) => {
                                    console.log(res);
                                    if (res.message) {
                                        let toast = this.toastCtrl.create({
                                            message: res.message,
